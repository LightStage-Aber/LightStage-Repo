#!/bin/bash
#
# -- Run as: ./run_edge_range_test_evenness_scaling.sh 1> test_output/stdout.txt 2> test_output/stderr.txt
#
errecho(){ >&2 echo $@; }

# Declare test ranges:
declare -a range_edges=$(seq 2 1 10)  #("1.25") #$(seq 3 90)  #e.g. ("44" "45")

# Declare test output location:
result_set="test_output/Edge_Results_RangeTest_"$(date -d "today" +"%Y-%m-%d_%H")"-00"
...
function run_range_test(){
    for n in $range_edges
    do
        leds=$(python -c "x=$n;print((265*(x-1))+91);")
        echo "./run_edge_evenness_scaling_trial.sh $n $leds"
        if [ $? -ne 0 ]; then
            errecho ""; errecho " -------------- "
            errecho "./run_edge_evenness_scaling_trial.sh $n $leds failed."
            errecho "Exiting."
            exit 1;
        fi
    done
